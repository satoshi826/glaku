import {Camera, Core, Loop, Program, Renderer, Vao, box, normalize, Model, Vec3} from 'glaku'
...
export const main = (canvas: HTMLCanvasElement | OffscreenCanvas, pixelRatio: number) => {
  ...
  const models = range(CUBE_NUM).map(() => new Model({
    ...
    rotation: {axis: normalize([random(-1, 1), random(-1, 1), random(-1, 1)]) as Vec3, angle: 0},
